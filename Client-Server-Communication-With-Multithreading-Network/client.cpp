...
void file_thread_function(string &filename, BoundedBuffer &request_buffer, TCPRequestChannel *&chan, int buffersize) {
...
void worker_thread_function(BoundedBuffer &request_buffer, BoundedBuffer &response_buffer, TCPRequestChannel *&worker_channel, int buffersize, string &filename, ofstream &outfile) {
...
void histogram_thread_function(BoundedBuffer &response_buffer, HistogramCollection &hc, datamsg quitter) {
    while (true) {
        ...
        hc.update(((datamsg*)response)->person, ((datamsg*)response)->ecgno);
    }
    return;
}
...
int main (int argc, char* argv[]) {
    ...
    int r = 0;		//default port
	string f = "";	// name of file to be transferred
    string a = "";  //name of IP address
    // read arguments
    int opt;
    ...
    TCPRequestChannel** worker_channels = new TCPRequestChannel*[w];
    //idk but it makes it work
    int largest=w;
    for (int i = 0; i < w; i++) {
        worker_channels[i] = new TCPRequestChannel(a, r);
    }
